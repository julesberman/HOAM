...
from hoam.dnn import DNN
from hoam.utils import init_net, merge, split
...
def build_colora(
    ...
    # combine u and h networks
    def s_fn(mu, x, t, params):
        mu_t = jnp.concatenate([mu, t])
        theta, psi = params
        phi = h_fn(psi, mu_t)
        return jnp.squeeze(u_hat_fn(theta, phi, x))

    params_init = (theta_init, psi_init)
